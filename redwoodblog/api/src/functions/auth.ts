...
export const handler = async (event, localContext) => {
  ...
  const signupOptions = {
    ...
    handler: (params) => {
      const { username, hashedPassword, salt, userAttributes } = params
      console.log({ params })
  ...
  const authHandler = new DbAuthHandler(event, localContext, {
